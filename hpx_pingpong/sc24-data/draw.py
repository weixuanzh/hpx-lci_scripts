...
def batch(df):
    ...
    df1_tmp = df[df.apply(lambda row:
                          ...
                          row["nsteps"] == 1 and
                          row["name"] in ["mpi", "lci-ucx", "lci-ibv", "mpi_a"] and
                          row["pingpong_config_name"] == "flood",
                          axis=1)]
    df1 = df1_tmp.copy()
    plot(df1, "nthreads", "msg_rate(K/s)", "name", None,
         dirname=dirname, filename="flood-8b", base="lci-ibv", smaller_is_better=False, with_error=True,
    ...
    df1 = df1_tmp.copy()
    plot(df1, "nthreads", "msg_rate(K/s)", "name", None,
         dirname=dirname, filename="flood-16kb", base="lci-ibv", smaller_is_better=False, with_error=True,
         x_label="Thread Number", y_label="Message Rate (K/s)")
    ...
    plot(df1, "nchains", "latency(us)", "name", None,
         dirname=dirname, filename="nchains-8b", base="lci-ibv", smaller_is_better=True, with_error=True,
    ...
    df1 = df1_tmp.copy()
    plot(df1, "nchains", "latency(us)", "name", None,
         dirname=dirname, filename="nchains-16kb", base="lci-ibv", smaller_is_better=True, with_error=True,
         x_label="nchains", y_label="Latency (us)")
