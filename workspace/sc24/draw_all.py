...
def draw_line(dfs, x_key, y_key, tag_key, title,
    ...
    for ax, platform, df in zip(axs, platforms, dfs):
        ...
        if platform == platforms[0]:
            if ax2:
                ...
                fig.legend(lines1 + lines2, labels1 + labels2, loc="center", bbox_to_anchor = (0.5, 1), ncols=5, fontsize=14)
            else:
                ax.legend(loc="center", bbox_to_anchor = (0.5, 1), ncols=5, fontsize=14)
    ...
    dirname = os.path.join(output_path, "mpi_vs_lci")
...
def draw_bar(configs, dataset, labels, filename, title=None, legend_loc="best", legend_fn=None, with_error=True):
    if title is None:
        title = filename
    ...
    if not os.path.exists(output_path):
        os.mkdir(output_path)
    dirname = os.path.join(output_path, "config")
    if not os.path.exists(dirname):
        os.mkdir(dirname)
    output_png_name = os.path.join(dirname, "{}.png".format(filename))
    fig.savefig(output_png_name, bbox_inches='tight')
...
def extract_microbenchmark(df_mbench, name, platform, criteria_fn=None):
    ...
    message_rate_16kib = safe_loc(df1_tmp[df1_tmp.apply(lambda row:
                                            ...
                                            row["nchains"] == platform_config[platform]["flood_16kb_nchains"] and
                                            row["nsteps"] == 1 and
                                            (row["nthreads"] == platform_config[platform]["nthreads"]
                                             or pd.isna(row["nthreads"])) and
                                            row["pingpong_config_name"] == "flood",
...
def apply_mask(l, mask):
    if mask:
        ...
        return l
def draw_diff(dfs, base, configs, filename, title=None, legend_loc="best", legend_fn=None, label_mask=None, criteria_fn=None):
    labels = ["Message Rate\n(8B)", "Message Rate\n(16KiB)", "1 / Latency\n(8B)", "1 / Latency\n(16KiB)", "Efficiency", "1 / Octotiger\n(s)"]
    ...
    draw_bar(configs, dataset, labels, filename, title, legend_loc, legend_fn, with_error=with_error)
...
def batch(dfs):
    ...
    draw_line([df[0] for df in dfs], "nthreads", "msg_rate(K/s)", "name", None,
              filename="flood-8", base="lci", smaller_is_better=False, with_error=True, sort_key=sort_key,
              ...
              criteria_fn=lambda row: row["nbytes"] == 8 and
                                      ...
                                      row["pingpong_config_name"] == "flood")
    draw_line([df[0] for df in dfs], "nthreads", "msg_rate(K/s)", "name", None,
              filename="flood-16384", base="lci", smaller_is_better=False, with_error=True, sort_key=sort_key,
              x_label="Thread Number", y_label="Message Rate (K/s)",
              xscale="log", yscale="log",
              criteria_fn=lambda row: row["nbytes"] == 16384 and
                                      ...
                                      row["name"] in ["mpi", "mpi_a", "lci"] and
                                      row["pingpong_config_name"] == "flood")
    draw_line([df[0] for df in dfs], "nchains", "latency(us)", "name", None,
              filename="nchains-8", base="lci", with_error=True, sort_key=sort_key,
              ...
              criteria_fn=lambda row: row["nbytes"] == 8 and
                                      ...
                                      row["pingpong_config_name"] == "nchains")
    draw_line([df[0] for df in dfs], "nchains", "latency(us)", "name", None,
              filename="nchains-16384", base="lci", with_error=True, sort_key=sort_key,
              x_label="Chain Number", y_label="Latency (us)",
              xscale="log", yscale="log",
              criteria_fn=lambda row: row["nbytes"] == 16384 and
                                      row["nsteps"] > 1 and
    ...
    draw_line([df[0] for df in dfs], "task_comp_time", "efficiency", "name", None,
              filename="efficiency", base="lci", smaller_is_better=False, with_error=True, sort_key=sort_key,
    ...
    draw_line([df[1] for df in dfs], "nnodes", "Total(s)", "name", None, filename="brief",
    ...
    draw_diff(dfs, "lci",
              ["lci_header_sync_single_poll",
               "lci_header_sync_single_nolock",
                 "lci_header_sync_single_nolock_poll",
                 "lci_header_sync_single",
                 "lci_sendrecv"],
                "incoming", title="Polling for unexpected incoming messages",
              legend_loc="upper right", legend_fn=lambda x: x[4:],
    ...
    draw_diff(dfs, "lci",
              ["lci_followup_2queue",
               ...
               "lci_followup_sync_poll"],
                "followup", title="Managing a large number of pending operations",
                legend_loc="upper left", legend_fn=lambda x: x[13:])
    draw_diff(dfs, "lci",
              ["lci_pin", "lci_pthread", "lci_pthread_worker"],
                "progress", title="Different ways of driving the progress engine",
                legend_loc="upper left", legend_fn=lambda x: x[4:])
    draw_diff(dfs,
                "lci_mt_d1_c1", ["lci_{}_d{}_c1".format(x1, x2) for x1 in ["mt", "pin"] for x2 in [1, 2, 4, 8, 16]],
                "ndevices", title="Replication of communication resources", legend_fn=lambda x: x[4:-3])
    draw_diff(dfs,
                "lci_mt_d1_c1", ["lci_mt_d{}_c1".format(x) for x in [1, 2, 4, 8, 16]] +
              ["lci_{}_d{}".format(x1, x2) for x1 in ["global", "global_b"] for x2 in [1, 2, 4, 8, 16]],
                "device_lock", title="Fine-grained locks on communication resources", legend_fn=lambda x: x[4:])
    draw_diff(dfs,
                "lci", ["lci_a"],
                "aggregation", title="Aggregation", legend_fn=lambda x: x[4:])
