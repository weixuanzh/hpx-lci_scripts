...
def plot(df, x_key, y_key, tag_key, title,
    ...
    if ax2:
        ...
        ax2.legend(lines1 + lines2, labels1 + labels2, loc='lower left', ncol=1, fancybox=True)
    else:
        ax.legend()
...
def batch(df):
    ...
    plot(df, "Size(B)", "message rate(mps)", "name", None,
         dirname=dirname, filename="lci_vs_mpi", base="lci",
         x_label="Size (B)", y_label="Message Rate (Mps)",
         smaller_is_better=False, with_error=True)
